    ...
    void bfgs()
    {
        ...
        for (int epoch=0; epoch<nepoch; epoch++)
        {
            ...
            for(int i=1;i<=tot;i++){
                relation_tmp = relation_vec;
                entity_tmp = entity_vec;
                for(int j=0;j<pre[i].ycl_h.size();j++){
                    if (ok[make_pair(pre[i].ycl_h[j],pre[i].ycl_r[j])].count(pre[i].ycl_t[j])>0){
                        continue;
                    }
                    int opt=0;
                    if(pre[i].h!=pre[i].ycl_h[j]) opt++;
                    if(pre[i].r!=pre[i].ycl_r[j]) opt++;
                    if(pre[i].t!=pre[i].ycl_t[j]) opt++;
                    if(opt!=1) continue;
                    train_kb(pre[i].h,pre[i].t,pre[i].r,
                    pre[i].ycl_h[j],pre[i].ycl_t[j],pre[i].ycl_r[j]);
                    norm(relation_tmp[pre[i].r]);
                    norm(entity_tmp[pre[i].h]);
                    norm(entity_tmp[pre[i].t]);
                    if(pre[i].ycl_h[j]!=pre[i].h){
                        norm(entity_tmp[pre[i].ycl_h[j]]);
                    }
                    if(pre[i].ycl_t[j]!=pre[i].t){
                        norm(entity_tmp[pre[i].ycl_t[j]]);
                    }
                }
                relation_vec = relation_tmp;
                entity_vec = entity_tmp;
...
void init_pre(){
    FILE *f1=fopen("./add.txt","r");
    assert(f1);
    int st;
    while(fscanf(f1,"%s%s%s%d",bufh,bufr,buft,&st)==4){
        string hs=bufh;
        string rs=bufr;
        string ts=buft;
        if(entity2id.count(hs)==0) assert(0);
        if(entity2id.count(ts)==0) assert(0);
        if(relation2id.count(rs)==0) assert(0);
        if(st){
            tot++;
            pre[tot].ycl_h.clear();
            pre[tot].ycl_r.clear();
            pre[tot].ycl_t.clear();
            pre[tot].h=entity2id[hs];
            pre[tot].r=relation2id[rs];
            pre[tot].t=entity2id[ts];
            train.add(entity2id[hs],entity2id[ts],relation2id[rs]);
        }else{
            pre[tot].ycl_h.push_back(entity2id[hs]);
            pre[tot].ycl_r.push_back(relation2id[rs]);
            pre[tot].ycl_t.push_back(entity2id[ts]);
        }
    }
}
